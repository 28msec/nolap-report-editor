angular.module('layoutmodel', [])  
  .directive('layoutmodel', function() {
    return {
      restrict: 'E',
      templateUrl: '/report/preview/layoutmodel.html',    
      scope: {
         layoutModel: '=model',
         "class" : '@',
         tableSet: '=table',
      ...
      controller: function ($scope, $element, $sce) {    	      	      	 
    	  ...
    	  if (!scope.layoutModel) {
    		  scope.table = [];
    		  scope.y_headerGroups = [];
              scope.x_headerGroups = [];
              scope.data = [];
              return;
    	  }
    	...
    	// Check if this component may be used to render the table    	      	     	  
          ...
          for (var hd in scope.table.TableHeaders.x)
        	  for (var grp in scope.table.TableHeaders.x[hd].GroupCells)
        		  scope.x_headerGroups.push(scope.table.TableHeaders.x[hd].GroupCells[grp]);
          
          var idx = 0;
          for (var hd in scope.table.TableHeaders.y)
        	  for (var grp in scope.table.TableHeaders.y[hd].GroupCells) {
        		  ...
        		  for (var cell in scope.table.TableHeaders.y[hd].GroupCells[grp])
        		  {
        			 var c = scope.table.TableHeaders.y[hd].GroupCells[grp][cell];
        			  ...
        			  if (scope.innerTitle == null && cell==0 && hd==0 && grp==0 && scope.table.TableHeaders.y.length==1 && scope.table.TableHeaders.y[hd].GroupCells.length==1) {        				 
        				  var empty = true;
        				  for (var d in scope.data[0]) if (scope.data[0][d] != null) empty = false;
        				  if (empty) {
            			    scope.innerTitle = c;
            			    scope.data.shift();
            			    continue;
        			 ...
        			 while (scope.y_headerGroups.length <= idx) scope.y_headerGroups.push([]);
        			 scope.y_headerGroups[idx].push(c);
        			 idx += (c.CellSpan || 1);
